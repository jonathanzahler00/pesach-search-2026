...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  const seasonEnded = isSeasonEnded();
  ...
  return (
    <html lang="en">
      <body className={`min-h-screen ${playfair.variable} ${dmSans.variable}`}>
            ...
            <Link href="/" className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-[9px] sm:text-[10px] text-primary-300 uppercase tracking-widest">
                  {seasonEnded ? '5787 updates' : '5786 · Multi-Source Guide'}
                </p>
              </div>
            </Link>
        ...
        <main
          className={`max-w-6xl mx-auto px-4 py-4 sm:py-6 page-transition ${seasonEnded ? 'pb-8' : 'pb-24 sm:pb-8'}`}
        >
          {children}
        </main>

        {!seasonEnded && <BottomNav />}
        {!seasonEnded && <FeedbackButton />}

        <Analytics />

        <footer
          className={`border-t border-primary-100 mt-8 py-6 text-center text-xs text-primary-400 ${seasonEnded ? 'pb-6' : 'pb-24 sm:pb-6'}`}
        >
          {seasonEnded ? (
            <>
              <p>Thank you for using Pesach Search. Guides reflected OU, CRC, Star-K, and COR for 5786 (2026).</p>
              <p className="mt-1">This tool is for reference only. Always consult your rabbi for halachic questions.</p>
            </>
          ) : (
            <>
